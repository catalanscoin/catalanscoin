...
./catalans-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./catalans-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100  # cat has 2 decimals so 100 is the divide/multiply factor
minAmount = 100 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 3
fee = 10 # atomic units, cat would be 0.10 as the tx network fee
